...
class GenerateGrade extends Page
{
    ...
    {
        if (empty($this->grid_ids)) {
            Notification::make()->title('Nada para validar').body('Gere a grade antes de validar.').warning()->send();
            return;
        }

        // 1) Conflitos de disponibilidade atual
        $conflicts = [];
        for ($a = 0; $a < 5; $a++) {
            $row = $this->grid_ids[$a] ?? [];
            for ($d = 0; $d < 5; $d++) {
        ...
        if (!empty($conflicts)) {
            Notification::make()->title('Conflitos detectados').warning()->body(
                'Ajuste disponibilidades/seleções. Ex.: ' . implode(', ', array_slice($conflicts, 0, 3)) . (count($conflicts) > 3 ? '...' : '')
            )->send();
            return;
        }

        // 2) Completude: desejadas x alocadas (por professor)
        $selectedPids = collect($this->selected)
            ->filter(fn ($r) => !empty($r['professor_id']))
            ->pluck('professor_id')->map(fn ($v) => (int) $v)->unique()->values();
        if ($selectedPids->isEmpty()) {
            Notification::make()->title('Seleção vazia').warning()->body('Selecione professores antes de validar.')->send();
            return;
        }
        $professors = Professor::with('materia')->whereIn('id', $selectedPids)->get();
        $desiredByProf = $professors->mapWithKeys(fn ($p) => [$p->id => (int) ($p->materia->quant_aulas ?? 0)])->all();
        $placedByProf = array_fill_keys($selectedPids->all(), 0);
        for ($a = 0; $a < 5; $a++) {
            $row = $this->grid_ids[$a] ?? [];
            for ($d = 0; $d < 5; $d++) {
                $cell = $row[$d] ?? null;
                if (is_array($cell) && !empty($cell['professor_id'])) {
                    $pid = (int) $cell['professor_id'];
                    if (array_key_exists($pid, $placedByProf)) $placedByProf[$pid]++;
                }
            }
        }
        $problems = [];
        foreach ($desiredByProf as $pid => $need) {
            $placed = (int) ($placedByProf[$pid] ?? 0);
            $missing = max(0, $need - $placed);
            if ($missing > 0) {
                $p = $professors->firstWhere('id', $pid);
                $problems[] = sprintf('%s (%s): faltam %d', $p?->nome ?? 'Professor', $p?->materia?->nome ?? 'Matéria', $missing);
            }
        }

        if (!empty($problems)) {
            Notification::make()->title('Pendências na grade')->warning()->body(implode(' • ', array_slice($problems, 0, 3)) . (count($problems) > 3 ? '...' : ''))->send();
            return;
        }

        Notification::make()->title('Grade válida').success()->send();
    }
