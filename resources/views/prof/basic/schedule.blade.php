...
<body>
    <div style="max-width:1040px;margin:24px auto;">
        <div class="bar" style="margin-bottom:16px;">
            ...
            <a href="{{ route('prof.basic.dashboard') }}" class="btn">Dashboard</a>
        </div>
        ...
        <div x-data="schedule({
            ...
            <form method="POST" action="{{ route('prof.basic.schedule.save') }}" @submit.prevent="submit"
                x-ref="form">
                @csrf
                                    ...
                                    <td>
                                        <div class="cell"
                                            ...
                                            <div style="font-weight:600; margin-bottom:6px;"
                                                x-text="grid[{{ $r }}][{{ $c }}] ? 'Aula disponível' : 'Aula indisponível'">
                                            </div>
                                            <span class="pill"
                                                :class="grid[{{ $r }}][{{ $c }}] ? 'pill-on' :
                                                    'pill-off'">
                                                <span
                                                    x-text="grid[{{ $r }}][{{ $c }}] ? 'Disponível' : 'Indisponível'"></span>
                                            </span>
                                        </div>
                                    </td>
                                @endfor
    ...
    <script>
        function schedule({
            ...
            return {
                ...
                submit(e) {
                    // Directly set the value of the hidden input before submitting
