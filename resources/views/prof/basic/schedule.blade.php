...
@section('content')
  ...
  <div class="max-w-5xl mx-auto">
    ...
    <x-ui.card class="p-4 mb-4">
      <div class="text-sm text-slate-600 dark:text-slate-300">Legenda:</div>
      <div class="mt-2 flex flex-wrap gap-2 text-xs">
        <span class="inline-flex items-center gap-2 rounded-full px-2.5 py-0.5 pill-off">Indisponível</span>
        <span class="inline-flex items-center gap-2 rounded-full px-2.5 py-0.5 pill-on">Disponível</span>
        <span class="inline-flex items-center gap-2 rounded-full px-2.5 py-0.5 pill-aula">Aula (travado)</span>
      </div>
    </x-ui.card>
      ...
      <x-ui.card class="p-5">
        <form method="POST" action="{{ route('prof.basic.schedule.save', ['periodo' => $periodo ?? 'manha']) }}" @submit.prevent="submit" x-ref="form" class="space-y-5">
              ...
              <tbody>
                @for ($r = 0; $r < $rows; $r++)
                  <tr>
                    ...
                    @for ($c = 0; $c < $cols; $c++)
                      <td class="align-top">
                        <div class="rounded-xl border px-4 py-3 h-24 flex items-center justify-center cursor-pointer select-none transition hover:ring-1 hover:ring-slate-300"
          ...
          <div class="flex flex-wrap justify-end gap-3">
            ...
            <x-ui.button type="button" variant="secondary" @click="setAll(0)">Limpar tudo</x-ui.button>
  ...
  <script>
    function schedule({ rows, cols, initial }) {
      ...
      return {
        rows, cols, grid, payload: JSON.stringify(grid),
        toggle(i, j) {
